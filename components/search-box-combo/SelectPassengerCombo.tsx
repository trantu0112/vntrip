...
import { setAdultCount, setChildCount, setInfantCount } from '../../store/combo/action'
import { toggleClassNoScroll } from '../../utils/common'

const SelectPassengerCombo: React.FC = React.memo(() => {
    ...
    const handleChangePassenger = (type1: 'adult' | 'child' | 'infant') => (type2: 'descrease' | 'increase') => () => {
        ...
        switch (type1 + '-' + type2) {
            case 'adult-descrease':
                ...
                if (newAdtCount < childCount) {
                    dispatch(setChildCount(newAdtCount))
                }
                break
            case 'adult-increase':
                if (adultCount < 6) {
    ...
    return (
        <div className="passengerBox">
            <p className="size-16 semibold">{t('combo:Bạn đi bao người')}?</p>
...
export default SelectPassengerCombo
